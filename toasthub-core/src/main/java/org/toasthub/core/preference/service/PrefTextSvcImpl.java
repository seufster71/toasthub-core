...
@Service("PrefTextSvc")
public class PrefTextSvcImpl implements ServiceProcessor, PrefTextSvc {
	...
	public void itemCount(RestRequest request, RestResponse response) {
		try {
			prefTextDao.itemCount(request, response);
		} catch (Exception e) {
	...
	public void items(RestRequest request, RestResponse response) {
		try {
			prefTextDao.items(request, response);
			// null values because they are lazy loaded but empty
			List<PrefTextName> items = (List<PrefTextName>) response.getParam(GlobalConstant.ITEMS);
			for (PrefTextName item : items) {
				item.setValues(null);
			}
		} catch (Exception e) {
	...
	public void item(RestRequest request, RestResponse response) {
		try {
			prefTextDao.item(request, response);
		} catch (Exception e) {
